...
(defparameter author "Matthew Carter")
...
(defun markup-file (file-name)
  ...
  (let ((html ""))
    (with-open-file (stream file-name) ;; eval any lisp
      (loop for line = (read stream nil 'eof)
         until (not (listp line))
         do (eval line))
      (loop for line = (read-line stream nil 'eof) ;; grab html
         until (eq line 'eof)
...
(defun main (file-name)
  ...
  (let* ((base-file-name (cl-ppcre:regex-replace-all ".*\/_posts\/(.*)" file-name "\\1"))
         (file-date (cl-ppcre:regex-replace-all "(\\d{4}-\\d{2}-\\d{2}).*" base-file-name "\\1")))
